...
class CS_Rectangle_View(BoxLayout, AView):
    ...
    def update_all_graph(self):
        ...
        for layer in self.layers:
            ...
            layer.rect.points = self.draw_layer(
                self.cross_section_width / 2, layer.y_coordinate, self.cross_section_width, layer._height)
        ...
        for rectangle in self.layers:
            if rectangle.focus and rectangle.mouse_within(x_coordinate, y_coordinate):
                if y_coordinate > rectangle._height / 2 and y_coordinate < self.cross_section_height - rectangle._height / 2:
                    rectangle.rect.points = self.draw_layer(
                        self.cross_section_width / 2, y_coordinate, self.cross_section_width, rectangle._height)
                    rectangle.set_y_coordinate(y_coordinate)
                    return
                elif y_coordinate < rectangle._height / 2:
                    rectangle.rect.points = self.draw_layer(
                        self.cross_section_width / 2, rectangle._height / 2, self.cross_section_width, rectangle._height)
                    rectangle.set_y_coordinate(rectangle._height / 2)
                    return
                elif y_coordinate > self.cross_section_height - rectangle._height / 2:
                    rectangle.rect.points = self.draw_layer(
                        self.cross_section_width / 2, self.cross_section_height - rectangle._height / 2, self.cross_section_width, rectangle._height)
                    rectangle.set_y_coordinate(
                        self.cross_section_height - rectangle._height / 2)
                    return
    ...
    def add_layer(self, value, material):
        ...
        cur = Layer_Rectangle(self.cross_section_width / 2, self.cross_section_height - height / 2., height,
                              self.cross_section_width, [next(colorcycler), next(colorcycler), next(colorcycler), 1], value)
