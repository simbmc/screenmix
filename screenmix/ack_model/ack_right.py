...
class Ack_Right(GridLayout):
    ...
    def update(self):
        ...
        for layer in free_places:
            self.rect = FilledRect(xrange=[0, self.cross_section.concrete_stiffness * self.slider.value],
                                   yrange=[layer[0], layer[1]],
                                   color=[255, 255, 255])
            print(self.cross_section.calculate_strain_of_concrete() * self.slider.value)
            self.graph.add_plot(self.rect)
        # draw the layers
        for layer in self.cross_section.view.layers:
            layer.rect = FilledRect(xrange=[0, layer.material.stiffness * self.slider.value],
                                    yrange=[
